...
```
cryptsetup luksOpen --key-file /etc/lvmkey /dev/groupA/home securebackuphome
```
...
- securebackuphome: 解锁后映射到 /dev/mapper/securebackuphome 这个设备节点

该命令执行后，系统会创建一个新的映射设备：

```
/dev/mapper/securebackuphome
```

你可以通过它访问解密后的内容，比如挂载它：

```
sudo mount /dev/mapper/securebackuphome /mnt/secure_home
```
