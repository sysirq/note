...
# Check if Installation is done which includes creating volumes, formating
# and encrypting them. If done, lvms are usable and can be directly mounted.
isInstallInProgress "$system" # 该参数为 Linux 内核启动参数 ， 对于虚拟机来说 为A
```
...
isInstallInProgress() {
    ...
    /bin/mount $boot_partition $boot_dir
    if [ $? != 0 ]
    then
        #echo_msg 1 "Unable to mount the boot device, aborting"
        echo "Unable to mount the boot device, aborting"
    fi

    if [ -e "$boot_dir/.install_done" ]
    then
        retval=0
    fi
