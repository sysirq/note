...
- 查询：具有“y”值为“q”的KRPC消息字典，包含两个额外的键；“q”和“a”，"q"键有一个包含查询方法名称的字符串值。键"a"有一个包含查询命名参数的字典值

- 响应：具有“y”值为“r”的KRPC消息字典，包含一个额外的键“r”。键“r”的值是一个包含命名返回值的字典。在查询成功完成后发送响应消息。代码需要更加tid进行识别是那种消息类型（eg: find_node、get_peers等）
...
“通过局部收敛实现全局收敛”是 DHT 网络设计中的一个核心理念，尤其体现在 Kademlia、Mainline DHT 等实现中

### 什么是“局部收敛”

在 DHT 中，每个节点维护一张路由表（K-buckets），这张表按照与自身 ID 的“距离”划分为若干个区间。

局部收敛（Local Convergence） 指的是：节点在自身 ID 附近（即前缀相似的区间）内逐渐形成稳定且准确的节点集合。

也就是说，每个节点都在努力“认识”自己附近的节点是谁。

例如：

```
我的 ID: 0xA3F2...
我会优先维护距离我最近的节点（例如 0xA3E9...、0xA3F8...）
```

这种邻域更新靠以下机制实现：

- 定期向邻近区间发送 FIND_NODE；
- 当 bucket 满时，优先保留活跃节点；
- 失效节点被新节点替换；
- id[19] = random() & 0xFF; 等逻辑制造局部扰动。

这样每个节点的邻域不断变得更真实、更精确 —— 这就是“局部收敛”。

### 全局收敛的形成机制

每个节点都在局部地进行收敛维护，而这些“局部区域”在 ID 空间中部分重叠。

举个例子：

```
节点A 维护区间 [0xA3E0, 0xA3FF]
节点B 维护区间 [0xA3F0, 0xA410]
节点C 维护区间 [0xA400, 0xA420]
```

这些区间是连续且交叠的。

当 A、B、C 各自更新邻域时，它们的邻域信息会互相传播：
...
Hajime - And the Mainline DHT

https://www.first.org/resources/papers/conf2017/Hajime-the-Mainline-DHT.pdf