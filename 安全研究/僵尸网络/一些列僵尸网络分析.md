...
mozi通过version识别流量是否由其节点发送

mozi dht version生成算法：

- 1byte 随机字符 | 1byte magic字符 | 剩下两个字符通过算法计算得到 |
...
读取 socket 数据但不从缓冲区移除，判断是否数据已经完整，完整了才进行消息解析，不完整的话，会继续等待消息完整收到


### c2隐藏

通过反调试（Anti-Debugging）和字符串混淆技术将真正的 C2 地址隐藏了起来

在 main.c 的 main() 函数中，初始化了一个假的服务器地址：
...
这里的FAKE_CNC_ADDR 与 FAKE_CNC_PORT 是一个毫无意义的随机地址

而mirai通过SIGTRAP 改变执行流程，从而更新真正的C2地址
...
假地址 -> 抛出断点信号 -> 触发反调试回调 -> 替换函数指针 -> 解密内存表 -> DNS 查询真实域名
