...
usr.tar.xz 
```
这四个文件解压，然后删除对应的tar.xz

删除 init （软连接到/sbin/init），创建init 到 /bin/init

```
sysirq@sysirq-machine:~/Work/Fortinet/rootfs/tmp$ ls -hla init
lrwxrwxrwx 1 root root 9 Aug  8 10:21 init -> /bin/init
```

用patch掉的init 替换 bin/init

创建rootfs:

```
sysirq@sysirq-machine:~/Work/Fortinet/rootfs/tmp$ ls 
bin  boot  data  data2  dev  etc  fortidev  init  lib  lib64  migadmin  node-scripts  proc  sbin  sys  tmp  usr  var
