...
start_kernel --> rest_init--> kernel_init --> kernel_init_freeable --> init_fos_ima-->sub_FFFFFFFF8055F845(这个函数会对/data/lib/libips 与 /data/lib/libav 做完整性检查，但是我们不会修改该函数，所以可以patch)-->sub_FFFFFFFF8055F338 存在完整性检查
...
- 用 .db.x 验证 .db 的完整性
- 读取.db 中的条目，计算条目指定文件hash与.db中的hash是否相等
