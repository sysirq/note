...
start_kernel --> rest_init--> kernel_init --> kernel_init_freeable --> init_fos_ima-->sub_FFFFFFFF8055F845(这个函数会对/data/lib/libips 与 /data/lib/libav 做完整性检查，但是我们不会修改这两个文件，所以可以不patch)-->sub_FFFFFFFF8055F338 存在完整性检查
    ...
    while ( 1 )
        {
          ...
          if ( (v49 & 4) != 0 )
            *(_DWORD *)v51 = 0;
          LOBYTE(v64) = v47;
          v52 = filp_open(v46, 0LL, 0LL);
          if ( v52 > 0xFFFFFFFFFFFFF000LL )
            JUMPOUT(0xFFFFFFFF8055FA4ALL);
          v58 = v52;
          v59 = ima_calc_file_hash(v52, (unsigned __int8 *)&v64);
          filp_close(v58, 0LL);
          if ( v59 )
            JUMPOUT(0xFFFFFFFF8055FA3BLL);
          if ( (unsigned int)memcmp(v46 + 68, v65, BYTE1(v64)) )
            JUMPOUT(0xFFFFFFFF8055FA82LL);
          strcmp((const char *)v46, "/data/hash_bin.sha256");
          v53 = *(__int64 **)(v46 + 136);
          v46 = (__int64)(v53 - 17);
          if ( v63 == v53 )
          {
            if ( v63 != (__int64 *)v63[0] )
              JUMPOUT(0xFFFFFFFF8055F7A7LL);
            JUMPOUT(0xFFFFFFFF8055F7F5LL);
          }
        }
```
...
- 读取.db 中的条目，计算条目指定文件hash与.db中的hash是否相等，不等则machine_halt

- 如果存在文件hash_bin.sha256 ， 则进一步进行处理， 此文件列出了系统上关键文件的hash值，将该hash值加入Linux IMA 系统中

hash值加入Linux IMA 函数为：

```c
__int64 *__fastcall sub_FFFFFFFF8055D6CE(__int64 a1)
{
  __int64 *v1; // r12
  __int64 v3; // rbx
  __int64 *v4; // r13
  ...
  v1 = (__int64 *)find_watch_file(a1);
  if ( v1 )
    return v1;
  v1 = (__int64 *)kmem_cache_alloc(qword_FFFFFFFF816C5760, 6291520LL);
  if ( !v1 )
    return v1;
  if ( _InterlockedCompareExchange(&dword_FFFFFFFF81887428, 255, 0) )
    queued_write_lock_slowpath(&dword_FFFFFFFF81887428);
  v3 = qword_FFFFFFFF81887430;
  if ( qword_FFFFFFFF81887430 )
  {
    v4 = &qword_FFFFFFFF81887430;
    while ( 1 )
    {
      v6 = strcmp(a1, v3 + 56);
      if ( v6 >= 0 )
  ...
  v4 = &qword_FFFFFFFF81887430;
LABEL_14:
  strlcpy(v1 + 7, a1, 512LL);
  *v1 = v3;
  v1[1] = 0LL;
  v1[2] = 0LL;
  *v4 = (__int64)v1;
  rb_insert_color(v1, &qword_FFFFFFFF81887430);
  LOBYTE(dword_FFFFFFFF81887428) = 0;
  return v1;
}
```
...
- sub_FFFFFFFF8055F338函数利用.db.x 对.db 进行完整性检查：nop 检查即可

![image-20240808165504654](images/image-20240808165504654.png)
...
解压rootfs中的bin.tar.xz,用patch掉的init替换 bin/init。然后重新生成 bin.tar.xz
...
```
sysirq@sysirq-machine:~/Work/Fortinet/rootfs/tmp$ ls -hl
total 169M
-rw-r--r-- 1 root   root    32M Aug  9 14:04 bin.tar.xz
drwxr-xr-x 2 root   root   4.0K Aug  9 13:57 boot
drwxr-xr-x 3 root   root   4.0K Aug  9 13:57 data
drwxr-xr-x 2 root   root   4.0K Aug  9 13:57 data2
drwxr-xr-x 8 root   root    20K Aug  9 13:57 dev
lrwxrwxrwx 1 root   root      8 Aug  9 13:57 etc -> data/etc
lrwxrwxrwx 1 root   root      1 Aug  9 13:57 fortidev -> /
lrwxrwxrwx 1 root   root     10 Aug  9 13:57 init -> /sbin/init
drwxr-xr-x 5 root   root   4.0K Aug  9 13:57 lib
lrwxrwxrwx 1 root   root      4 Aug  9 13:57 lib64 -> /lib
-r--r--r-- 1 root   root    14M Aug  9 13:57 migadmin.tar.xz
-r--r--r-- 1 root   root   639K Aug  9 13:57 node-scripts.tar.xz
drwxr-xr-x 2 root   root   4.0K Aug  9 13:57 proc
-rw-rw-r-- 1 sysirq sysirq 122M Aug  9 13:57 rootfs
drwxr-xr-x 2 root   root   4.0K Aug  9 13:57 sbin
drwxr-xr-x 2 root   root   4.0K Aug  9 13:57 sys
drwxr-xr-x 2 root   root   4.0K Aug  9 13:57 tmp
drwxr-xr-x 3 root   root   4.0K Aug  9 13:57 usr
-r--r--r-- 1 root   root   145K Aug  9 13:57 usr.tar.xz
drwxr-xr-x 9 root   root   4.0K Aug  9 13:57 var
...
qemu-nbd --connect=/dev/nbd0 fortios_patched.qcow2
fdisk -l /dev/nbd0
...
因为我们修改了init文件，所以需要重新生成init文件的sha256并修改该文件

- .db

因为我们修改了flatkc 、rootfs.gz、hash_bin.sha256 这三个文件，所以需要重新计算他们的SHA512并修改该文件
...
![image-20240809145230745](images/image-20240809145230745.png)
...
```
Breakpoint 8, 0xffffffff8055dec8 in ima_file_check ()
(gdb) x /s %rax
A syntax error in expression, near `%rax'.
(gdb) x /s 
0xffffffff803125e7 <kernel_restart>:	"UH\211\345ATI\211\374H\211\372\276\001"
(gdb) x /s $rax
0xffffffff8140c620:	"severity=alert msg=\"[Suspicious Executable File is missing hash](%s).\""
(gdb) print $rip
$3 = (void (*)()) 0xffffffff8055dec8 <ima_file_check+1691>
(gdb) print $r14
$4 = -60473061999094
(gdb) x /s $r14
0xffffc900049efe0a:	"/bin/hotplug"
(gdb) 
```



Suspicious Executable File is missing hash （/bin/hotplug）：原因是我直接替换了/init 使其为 /bin/init的软连接，原来的/init是 /sbin/init的软链接，/sbin/init会完成目录下.tar.xz的解压，但是我把bin.tar.xz直接解压了的，导致FFFFFFFF8055EE70 函数未完成/data/hash_bin.sha256读取并加载到IMA，而kobject_uevent_env（会通过call_usermodehelper_exec启动/bin/hotplug程序）又先于FFFFFFFF8055EE70执行，此时IMA中并没有/bin/hotplug的hash值，从而导致的无限重启。正常流程应该是：FFFFFFFF8055EE70完成IMA配置 --> /init --> /sbin/init 完成bin.tar.xz的解压 --> 此时才有 /bin/hotplug 二进制文件 --》 call_usermodehelper_exec启动/bin/hotplug程序



```
Breakpoint 3, 0xffffffff8055f6f3 in ima_pr_warning[cold] ()
(gdb) x /s $rdi
0xffff88806e387480:	"/data/hash_bin.sha256"
(gdb) c
Continuing.

Breakpoint 3, 0xffffffff8055f6f3 in ima_pr_warning[cold] ()
(gdb) x /s $rdi
0xffff88806e387180:	"/data/datafs.tar.gz"
(gdb) c
Continuing.

Breakpoint 3, 0xffffffff8055f6f3 in ima_pr_warning[cold] ()
(gdb) x /s $rdi
0xffff88806e387e40:	"/data/rootfs.gz"
(gdb) c
Continuing.

Breakpoint 3, 0xffffffff8055f6f3 in ima_pr_warning[cold] ()
(gdb) x /s $rdi
0xffff88806e3876c0:	"/data/flatkc"
(gdb) c
Continuing.
^C
...
**记得修改.db. 和 hash_bin.sha256文件**：修改 /bin/smartctl 以及 /bin/busybox的sha256sum。修改rootfs.gz、hash_bin.sha256的sha512sum 

### 网络配置

FortiGate禁止我们打开任意端口。这里的技巧是杀死合法的(和不必要的)服务并重用它们的端口。为此，我们配置接口以启用ssh和telnet:
...
![image-20240809170704825](images/image-20240809170704825.png)

可以看到我们编译的backdoor程序运行了。

用nc连接：

```
sysirq@sysirq-machine:~/Work/busybox/busybox-1.36.1$ nc 192.168.182.188 22
????????
/ # 
...
/ # busybox ls  
busybox ls
bin           etc           migadmin      rootfs        var
boot          fortidev      new_root      sbin
data          init          node-scripts  sys
data2         lib           proc          tmp
dev           lib64         root          usr
/ # 
```
...
下载gdb（https://ftp.gnu.org/gnu/gdb/）
...
```sh
/bin/busybox kill -9 $(/bin/busybox pidof telnetd) && gdbserver 0.0.0.0:23 --attach  $(/bin/busybox pidof sslvpnd)
```
...
新版本貌似在内核ptrace系统调用中做了手脚

```
busybox dmesg

invalid ptrace request (13) by pid: 2228
invalid ptrace request (15) by pid: 2228
invalid ptrace request (16896) by pid: 2228
invalid ptrace request (16896) by pid: 2536
invalid ptrace request (16896) by pid: 2363
invalid ptrace request (13) by pid: 2363
invalid ptrace request (15) by pid: 2363
invalid ptrace request (16896) by pid: 2363
invalid ptrace request (6) by pid: 2363
invalid ptrace request (16896) by pid: 2587
invalid ptrace request (16896) by pid: 1
invalid ptrace request (13) by pid: 1
```

```c
void __fastcall _x64_sys_ptrace(_QWORD *a1)
{
  __int64 v1; // r13
  ...
  v1 = a1[13];//pid
  v2 = a1[14];//ptrace_request
  if ( v2 > 32 )
  {
    if ( v2 != 0x4200 && v2 != 0x4205 )
      goto LABEL_4;
LABEL_24:
    if ( (unsigned int)((__int64 (__fastcall *)(void *, char *))__ratelimit)(&unk_FFFFFFFF81646380, aDoSysPtrace) )
      _x64_sys_ptrace_cold((__int64)&unk_FFFFFFFF81646380, (__int64)aDoSysPtrace, v10, v11, v12, v13);//打印invalid ptrace request (%ld) by pid: %ld\n
    return;
..................
```

去掉了 PTRACE_ATTACH	 与 PTRACE_SETOPTIONS的支持。我玩你妈！
...
{
  __int64 pid; // r13
  unsigned __int64 ptrace_request; // r12
  ...
  pid = a1[13];
  ptrace_request = a1[14];
  if ( ptrace_request > 32 )
  {
    if ( ptrace_request != 0x4200 && ptrace_request != 0x4205 )
      goto LABEL_4;
LABEL_24:
    if ( (unsigned int)((__int64 (__fastcall *)(void *, char *))__ratelimit)(&unk_FFFFFFFF81646380, aDoSysPtrace) )
      _x64_sys_ptrace_cold((__int64)&unk_FFFFFFFF81646380, (__int64)aDoSysPtrace, v10, v11, v12, v13);
    return;
  }
  if ( ptrace_request > 3 && ((1LL << ptrace_request) & 0x18500A270LL) != 0 )
    goto LABEL_24;
LABEL_4:
```

尝试patch，以去掉 if ( ptrace_request > 32 ) 和  if ( ptrace_request > 3 && ((1LL << ptrace_request) & 0x18500A270LL) != 0 )

![image-20240813111842417](images/image-20240813111842417.png)

修改之后，替换内核，（**注意修改.db 中 flatkc对应的sha512值**）
...
https://ioo0s.art/2023/02/07/利用VMware获取shell-进阶/
