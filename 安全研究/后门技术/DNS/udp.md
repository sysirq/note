...
int main() {
    ...
    // 构造DNS查询数据包
    int query_size = build_dns_query(hostname, dns_query, sizeof(dns_query));
    if (query_size < 0) {
        return 1;
    }

    // 送DNS查询数据包到DNS服务器并接收响应
    if(sendto(sockfd, dns_query, query_size, 0, (struct sockaddr *)&server_addr, sizeof(server_addr)) == -1){
        perror("sendto");
        goto fault;
    }

    query_size = recv(sockfd, dns_response, BUFFER_SIZE, 0);
    if(query_size == -1){
        perror("recvfrom");
        goto fault;
    }
