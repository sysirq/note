...
在这里，来自解码器上一层的输出（掩码自注意力层的输出,即使用 Mask 的注意力计算，保证每一个 token 只能使用该 token 之前的注意力分数）被用来构建 Query（查询向量），而编码器的最终输出则被用来构建 Key（键向量） 和 Value（值向量）。这使得解码器在生成每一个目标词时，都能回过头去精准检索源文本中最相关的部分。
