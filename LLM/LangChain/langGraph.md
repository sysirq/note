...
| 模式                       | 适用场景                                                     | 状态 Schema                       | 实现方式                                  |
| -------------------------- | ------------------------------------------------------------ | --------------------------------- | ----------------------------------------- |
| **在节点内调用子图**       | 父图与子图**state schema 不同**（无共享 key），或需要状态转换 | 需要写包装函数，手动映射父子state | 在节点函数中调用 `subgraph.invoke(...)`   |
| **直接把子图作为节点添加** | 父图与子图**共享state key**                                  | 直接传递编译后的子图              | `builder.add_node("node_name", subgraph)` |

### 子图持久化（Subgraph Persistence）

| 模式                       | `checkpointer=` | 行为                                                         |
| -------------------------- | --------------- | ------------------------------------------------------------ |
| **Per-invocation（默认）** | `None`          | 每次调用从干净状态开始，继承父图 checkpointer，支持 interrupt 和持久执行 |
| **Per-thread**             | `True`          | 同一 thread 上状态会累积，跨调用记忆                         |
| **Stateless**              | `False`         | 完全无检查点，像普通函数调用，不支持 interrupt               |

**1. Per-invocation（推荐默认）**
- 每次调用子图时，子图内部状态都是**全新的**（不记得上次调用的内容）。
- 但它会**继承父图的 checkpointer**，因此在**单次调用内部**仍然支持 `interrupt()` 暂停/恢复，以及持久执行。
- 多个并行调用不会冲突（每个调用有独立的 checkpoint namespace）。
- **最适合**：多 Agent 系统中，子 Agent 处理独立请求（比如“查订单”“总结文档”）。

**2. Per-thread**
- 子图状态会在**同一个 `thread_id` 上跨多次调用累积**。
- 第二次调用会接着上次的状态继续（比如记住之前聊过的内容）。
- 也支持 `interrupt()`。
- **注意限制**：不支持对同一个子图进行并行调用（会导致 checkpoint 冲突）。需要用中间件限制并行，或给不同子图分配稳定的命名空间。
- **最适合**：研究助手、编码助手等需要跨多轮对话积累上下文的场景。

**3. Stateless**
- 完全关闭检查点机制，子图像普通函数一样运行。
